...
#define JBACTIVE(OBJ)		(SCM_CELL_WORD_0 (OBJ) & (((int64_t) 1) << 16L))
#define ACTIVATEJB(x)	\
  (SCM_SET_CELL_WORD_0 ((x), (SCM_CELL_WORD_0 (x) | (((int64_t) 1) << 16L))))
...
pre_unwind_data_print (SCM closure, SCM port, scm_print_state *pstate SCM_UNUSED)
{
  ...
  sprintf (buf, "#<pre-unwind-data 0x%llx 0x%llx>", c->handler, c->handler_data);
  scm_puts (buf, port);
