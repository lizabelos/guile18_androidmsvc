...
inner_main (void *closure SCM_UNUSED, int argc, char **argv)
{
  ...
  /* the goal of this test the performance of scheme with big number (gmp) */
  scm_c_eval_string("(define (fact n) (if (= n 0) 1 (* n (fact (- n 1)))))");

  char command[1000];
  for (int i = 0; i < 200; i++) {
    sprintf(command, "(display (fact %d))", i);
    SCM res = scm_c_eval_string(command);
    printf("\n");
  }
