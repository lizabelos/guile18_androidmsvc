...
TYPE SCM_TO_TYPE_PROTO (SCM val) {
    if (SCM_I_INUMP (val)) {
        scm_t_signed_bits n = SCM_I_INUM (val);
        if (n >= 0 && ((scm_t_uintmax) n) >= TYPE_MIN && ((scm_t_uintmax) n) <= TYPE_MAX) {
            return n;
        }
        else {
            out_of_range:
            scm_i_range_error(val, scm_from_unsigned_integer(TYPE_MIN), scm_from_unsigned_integer(TYPE_MAX));
            return 0;
        }
        ...
        if (TYPE_MAX <= SCM_MOST_POSITIVE_FIXNUM)
            ...
            if (mpz_sizeinbase(SCM_I_BIG_MPZ (val), 2) > CHAR_BIT * sizeof(TYPE)) {
                goto out_of_range;
            }
