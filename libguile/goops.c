...
SCM
scm_i_get_keyword (SCM key, SCM l, int64_t len, SCM default_value, const char *subr)
{
  int64_t i;
...
#define FUNC_NAME s_scm_sys_initialize_object
{
  ...
  for (;
      ...
      if (!scm_is_null (SCM_CDR (slot_name)))
	{
	  ...
	  int64_t n = scm_ilength (SCM_CDR (slot_name));
	  if (n & 1) /* odd or -1 */
...
#define FUNC_NAME s_scm_sys_inherit_magic_x
{
  ...
  if (flags & SCM_CLASSF_ENTITY)
    ...
    {
      int64_t n = SCM_I_INUM (SCM_SLOT (class, scm_si_nfields));
#if 0
      /*
...
static SCM
wrap_init (SCM class, SCM *m, int64_t n)
{
  int64_t i;
...
static int64_t n_hell = 1;		/* one place for the evil one himself */
static int64_t hell_size = 4;
...
static SCM
scm_i_vector2list (SCM l, int64_t len)
{
  int64_t j;
...
static SCM
sort_applicable_methods (SCM method_list, int64_t size, SCM const *targs)
{
  int64_t i, j, incr;
...
SCM
scm_compute_applicable_methods (SCM gf, SCM args, int64_t len, int find_method_p)
{
  register int64_t i;
  int64_t count = 0;
...
#define FUNC_NAME s_scm_sys_method_more_specific_p
{
  SCM l, v, result;
  SCM *v_elts;
  int64_t i, len, m1_specs, m2_specs;
  scm_t_array_handle handle;
...
void
scm_make_port_classes (int64_t ptobnum, char *type_name)
{
